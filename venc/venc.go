...
func pre_parser(symbol_table Symbol_Table, code_original []Token, depth int, import_Handler func(string)Symbol_Table) (string, Symbol_Table) {
	code := make([]Token, 0)
	for _, code_obj := range code_original {
		code = append(code, clone_token(code_obj))
	}
		...
		if depth == 0 {
			...
			if code[i].Type == "sys" && code[i].string_value == "import" && (len(code)-i) >= 4 && code[i+1].Type == "string" && code[i+2].Type == "sys" && code[i+2].string_value == "as" && code[i+3].Type == "variable" && valid_var_name(code[i+3].string_value) && code[i+4].Type == "EOS" && !strings.Contains(code[i+1].string_value, "-") {
				...
				new_symbol_table := Symbol_Table{
					...
					File_Path: code[i+1].string_value,
				}
				...
				if str_index_in_arr(new_file_name, debug_keys) == -1 {
					new_symbol_table.files[new_file_name] = Symbol_Table{current_file: new_file_name, File_Path: code[i+1].string_value}
					new_symbol_table.current_file = new_file_name
					err_, new_file := build(new_symbol_table, tokens_, 1, import_Handler)
	...
	if len(tokens) == 1 {
			...
			if len(tokens[0].children) == 2 {
				...
				if len(variable_struct.fields[tokens[0].children[1].string_value]) != 0 {
					if nested_nested_tokens || tokens[0].children[0].Type == "variable" {
						...
						symbol_table = new_symbol_table
						new_symbol_table = var_init(variable_struct.fields[tokens[0].children[1].string_value], new_variable, symbol_table, function_name, false, true)
						symbol_table = new_symbol_table
						...
						symbol_table = new_symbol_table
						new_symbol_table = var_init(rhs_pretype, new_variable, symbol_table, function_name, false, true)
						symbol_table = new_symbol_table
				...
				symbol_table = new_symbol_table
				new_symbol_table = var_init(resolved_type, new_variable, symbol_table, function_name, false, true)
				symbol_table = new_symbol_table
				used_variables = append(used_variables, new_variable)
		...
		if tokens[0].Type == "lookup" {
			...
			if lhs_type[0] == "[" {
				...
				rhs, new_used_variables, new_symbol_table := expression_solver(tokens[0].children[1].children, function_name, symbol_table, false)
				used_variables = append(used_variables, new_used_variables...)
				symbol_table = new_symbol_table
...
func build(symbol_table Symbol_Table, tokens []Token, depth int, import_Handler func(string)Symbol_Table) (string, Symbol_Table) {
	file_path := symbol_table.File_Path
	symbol_table.data = append(symbol_table.data, tokens_to_string_constants(tokens)...)
	err, symbol_table := pre_parser(symbol_table, tokens, 0, import_Handler)
	if err != "" {
	...
	for _, fn := range symbol_table.functions {
		if depth == 0 {
			if strings.Split(fn.name, "-")[1] != "main" {
				...
				if strings.Split(fn.name, "-")[0] == file_path {
	...
	if depth == 0 {
		...
		for _, function := range symbol_table.functions {
			...
			if strings.Split(function.name, "-")[0] != file_path {
				continue
			}
