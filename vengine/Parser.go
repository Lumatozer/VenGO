	...
	for i:=0; i<len(code); i++ {
		...
		if code[i].Type=="variable" && code[i].Value=="set" {
			...
			if !Equal_Type(&program.Object_References[variable_Index].Object_Type, &Type{Raw_Type: INT_TYPE}) {
				return errors.New("expected type a literal integer while parsing set instruction")
			}
		...
		if code[i].Type=="variable" && code[i].Value=="str_set" {
			if len(code)-i<4 {
				return errors.New("invalid str_set instruction definition structure")
			}
			if code[i+1].Type!="variable" {
				return errors.New("invalid str_set instruction definition structure")
			}
			...
			if !Equal_Type(&program.Object_References[variable_Index].Object_Type, &Type{Raw_Type: STRING_TYPE}) {
				return errors.New("expected type a literal string while parsing str_set instruction")
			}
			if code[i+2].Type!="string" || code[i+3].Type!="semicolon" {
				return errors.New("invalid str_set instruction definition structure")
			}
			function.Constants.STRING = append(function.Constants.STRING, code[i+2].Value)
			function.Instructions = append(function.Instructions, []int{STRING_SET_INSTRUCTION, variable_Index, len(function.Constants.STRING)-1})
			i+=3
			continue
		}
